...
impl NaiveBayesClassifier {
    pub fn train(&mut self, message: &[Message]) {
        for msg in message.iter() {
            self.increment_message_classifications_count(msg);
            for token in tokenize(&msg.text.to_lowercase()) {
                self.tokens.insert(token.to_string());
                self.increment_token_count(token, msg.is_spam)
            }
        }
    }
...
impl NaiveBayesClassifier {
    ...
    fn probabilities_of_message(&self, message_tokens: HashSet<&str>) -> (f64, f64) {
        let mut log_prob_if_spam = 0f64;
        let mut log_prob_if_ham = 0f64;
        for token in self.tokens.iter() {
            ...
            if message_tokens.contains(token.as_str()) {
                ...
                log_prob_if_spam += (1 as f64 - prob_if_spam).ln();
                log_prob_if_ham += (1 as f64 - prob_if_ham).ln();
            }
        }
